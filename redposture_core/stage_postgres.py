...
def audit_postgres_targets(
    ...
    try:
        ...
        progress = (
            ...
            else start_audit_progress("POSTGRES", len(indexed_hosts), enabled=show_progress, leave=True)
        )
        deep_requested = bool(
            show_databases
            or show_tables
        ...
        for idx, _host in indexed_hosts:
            ...
            if output_format != "txt" and bool(record.get("is_postgres")):
...
def run_postgres_stage(args: argparse.Namespace, logger: AttemptLogger) -> int:
    ...
    if stream_to_stdout:
        if args.debug and args.output_format == "txt":
            console.info(
                f"postgres audit complete: total={total} anonymous={open_no_auth} "
