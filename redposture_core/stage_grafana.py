...
def audit_grafana_targets(
    ...
    try:
        ...
        for idx, _host in indexed_hosts:
            ...
            if output_format != "txt" and bool(record.get("is_grafana")):
...
def run_grafana_stage(args: argparse.Namespace, logger: AttemptLogger) -> int:
    ...
    if any(spec.scheme == "https" for spec in target_specs):
        console.error("grafana accepts only http:// URL targets for -t/--targets")
        return 2
    ...
    if args.debug and stream_to_stdout and args.output_format == "txt":
        ...
        if credential_file_entries is None and args.password is not None:
            mode_parts.append("provided-creds")
        mode = ",".join(mode_parts) if mode_parts else "detect-only"
        console.info(
            f"grafana audit started: hosts={len(hosts)} ports={len(execution_groups)} timeout={args.timeout}s "
