    ...
    *,
    ...
    try:
        ...
        deep_requested = bool(
            ...
            or show_volumes
            or show_system
            or (container_selector and exec_cmd)
        )

        def _detect_task(host: str) -> dict[str, Any]:
            return _audit_docker_host_stage(
                host,
                port,
                timeout,
                retries,
        ...
        pass_result = TwoPassAuditRunner(
            ...
            debug_emit=debug_emit,
            progress=progress,
            detected_name="docker",
        ).run(
            indexed_hosts,
            detect_task=_detect_task,
            deep_task=_deep_task,
            is_detected=lambda record: bool(record.get("is_docker")),
        ...
        for idx, _host in indexed_hosts:
            ...
            if status == "open_no_auth":
                ...
                fail += 1
            if output_format != "txt" and bool(record.get("is_docker")):
                _emit_line(out_fh, emit_line, _format_detect_record(record, output_format))
            suppress_fail_line = suppress_timeout_status_lines and output_format == "txt" and status == "fail"
            if not suppress_fail_line:
            ...
            for formatter in (
                ...
                for line in formatter(record, output_format):
                    _emit_line(out_fh, emit_line, line)
            for line in _format_exec_lines(record, output_format, debug=bool(debug_emit)):
                _emit_line(out_fh, emit_line, line)
            if logger is not None:
                logger.log(
                    ...
                    container_count=_container_count(record),
                    image_count=_image_count(record),
                )
...
def run_docker_stage(args: argparse.Namespace, logger: AttemptLogger) -> int:
    ...
    if bool(getattr(args, "container", None)) != bool(getattr(args, "exec_cmd", None)):
        ...
        return 2
    for attr, label in (("tls_ca", "--tls-ca"), ("tls_cert", "--tls-cert"), ("tls_key", "--tls-key")):
        value = getattr(args, attr, None)
        if value and not os.path.isfile(str(value)):
            console.error(f"{label} file does not exist: {value}")
            return 2
    ...
    try:
        ...
        elif bool(getattr(args, "_docker_port_explicit", False)):
            ports = [int(args.port)]
        else:
            ports = list(_DOCKER_DEFAULT_PORTS)
        hosts = collect_scan_targets(args.targets)
    except ValueError as exc:
        console.error(f"failed to parse targets: {exc}")
        return 2
    if not hosts:
        console.error("docker requires -t/--targets")
        return 2
    ...
    def emit_line(line: str) -> None:
        if args.output_format != "txt":
            ...
            return
        if line.startswith(_DOCKER_TAG) and all(token not in line for token in (" [*] ", " [+] ", " [-] ", " [!] ")):
            if console.render_tagged_payload_line(line, _DOCKER_TAG, payload_color="orange"):
                return
            console.plain(line, color="white")
            return
        if _render_colored_docker_line(console, line):
            return
        if args.debug:
            console.plain(line)
    ...
    if args.debug and args.output_format == "txt":
        mode = (
            ...
            if any(
                bool(getattr(args, attr, False)) for attr in ("containers", "images", "networks", "volumes", "system")
            )
            else "detect-only"
        )
        console.info(
            f"docker audit started: hosts={len(hosts)} ports={len(ports)} timeout={args.timeout}s "
    ...
    total = open_no_auth = valid = auth_required = failed = 0
    output_written = False
    try:
        for audit_port in ports:
            part_total, part_open, part_valid, part_auth, part_failed = audit_docker_targets(
                hosts=hosts_by_port[int(audit_port)],
                port=int(audit_port),
                ...
                show_containers=bool(args.containers),
                show_images=bool(args.images),
                show_networks=bool(args.networks),
                show_volumes=bool(args.volumes),
                show_system=bool(args.system),
                container_selector=getattr(args, "container", None),
    ...
    if not args.output and args.output_format == "txt":
        if args.debug:
            console.info(
                f"docker audit complete: total={total} anonymous={open_no_auth} valid={valid} auth={auth_required} fail={failed}"
            )
        elif total > 0 and open_no_auth == 0 and valid == 0 and auth_required == 0 and failed == total:
            console.warn(
                "all docker targets are unreachable; check host/port, network reachability, and service status"
            )
    elif args.debug:
        console.info(
            f"docker audit complete: total={total} anonymous={open_no_auth} valid={valid} auth={auth_required} "
            f"fail={failed} format={args.output_format} output={args.output}"
        )
    return 0
