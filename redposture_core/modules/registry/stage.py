...
def run_registry_stage(args: Any, logger: Any) -> int:
    ...
    if bool(getattr(args, "debug", False)):
        ...
        console.info("registry audit started:" + suffix)
    runner = AuditCommandRunner(args=args, spec=build_registry_spec(args), logger=logger, emit_line=console.plain)
    try:
    ...
    if result.detected_count == 0 and hasattr(console, "warn"):
        console.warn("all registry targets are unreachable")
    return 0
