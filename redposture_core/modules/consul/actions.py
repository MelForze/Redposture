"""consul audit actions and compatibility helpers."""
        ...
        try:
            ...
            if do_kv_list or do_kv_dump:
                ...
                if do_kv_dump:
                    ...
                    if kv_key is None:
                        kv_dump_items = _limit_dump_items(kv_dump_items, dump_limit)
            ...
            if show_services:
                ...
                elif dump_requested and isinstance(services_list, list):
                    services_list = _limit_dump_items(services_list, dump_limit)
                if dump_requested:
                    ...
                    elif dump_limit is not None:
                        names_for_details = names_for_details[:dump_limit]
            ...
            if show_agents:
                ...
                elif dump_requested and isinstance(agents_list, list):
                    agents_list = _limit_dump_items(agents_list, dump_limit)
            ...
            if show_checks:
                ...
                if isinstance(checks_payload, dict):
                    ...
                    if check_dump_id:
                        ...
                        checks_list = _limit_dump_items(checks_list, dump_limit) or []
                    checks_list_error = None
            ...
            if show_nodes:
                ...
                elif dump_requested and isinstance(nodes_list, list):
                    nodes_list = _limit_dump_items(nodes_list, dump_limit)
    ...
    *,
    ...
    def _invoke() -> dict[str, Any]:
        try:
            ...
            if not is_signature_compat_typeerror(exc, expected_keywords={"debug", "run_deep_checks", "dump_limit"}):
                raise
            return _audit_consul_host(
                host,
                port,
    ...
    *,
    ...
    for attempt in range(attempts):
        ...
        try:
            ...
            deep_record = _audit_consul_host_core(
                host,
                port,
                timeout,
                retries,
...
def record_from_mapping(payload: dict[str, Any]) -> AuditRecord:
    ...
    return AuditRecord.from_mapping(payload, module="consul", service="consul")


def host_hook(ctx: AuditHookContext) -> AuditRecord:
    """Typed host hook consumed by AuditCommandRunner."""

    return invoke_action_hook_from_args(sys.modules[__name__], module="consul", ctx=ctx)
