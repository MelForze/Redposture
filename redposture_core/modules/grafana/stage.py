...
def run_grafana_stage(args: Any, logger: Any) -> int:
    ...
    if bool(getattr(args, "debug", False)):
        ...
        console.info("grafana audit started:" + suffix)
    runner = AuditCommandRunner(args=args, spec=build_grafana_spec(args), logger=logger, emit_line=console.plain)
    try:
    ...
    if result.detected_count == 0 and hasattr(console, "warn"):
        console.warn("all grafana targets are unreachable")
    return 0
