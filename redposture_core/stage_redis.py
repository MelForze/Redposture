...
def audit_redis_targets(
    ...
    try:
        ...
        pass_result = TwoPassAuditRunner(
            ...
            is_detected=lambda record: bool(record.get("is_redis")),
            deep_gate=_deep_gate,
            emit_detect=_emit_detect,
            merge_records=_merge_stage2_record,
            not_detected_reason="not_redis",
        )

        for idx, _host in indexed_hosts:
            emit_record = pass_result.final_records[idx]
...
def run_redis_stage(args: argparse.Namespace, logger: AttemptLogger) -> int:
    ...
    if use_single_global_progress:
        progress_total = progress_total_from_groups(hosts_by_port.values(), len(credential_runs))
        outer_progress = start_command_progress(args, "REDIS", progress_total, enabled=True, leave=True)
