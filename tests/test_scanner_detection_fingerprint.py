...
def test_scan_uses_fingerprint_tiebreak_for_conflicting_markers(monkeypatch) -> None:
    called_urls: list[str] = []
...
def test_scan_returns_unknown_for_unresolved_conflict(monkeypatch) -> None:
    def fake_http_get_details(url: str, timeout: float, retries: int = 1, **kwargs) -> dict[str, object]:
        _ = (timeout, retries)
        if url.endswith("/metrics"):
            return {
...
def test_scan_uses_marker_fallback_when_top_candidate_has_stronger_metrics(monkeypatch) -> None:
    called_urls: list[str] = []
...
def test_scan_detects_single_weak_candidate_without_fingerprint_endpoints(monkeypatch) -> None:
    called_urls: list[str] = []
...
def test_scan_uses_unique_port_fallback_for_generic_prometheus_metrics(monkeypatch) -> None:
    def fake_http_get_details(url: str, timeout: float, retries: int = 1, **kwargs) -> dict[str, object]:
        _ = (timeout, retries)
        if url.endswith("/metrics"):
            return {
...
def test_scan_detects_node_exporter_from_production_node_metrics(monkeypatch) -> None:
    def fake_http_get_details(url: str, timeout: float, retries: int = 1, **kwargs) -> dict[str, object]:
...
def test_scan_does_not_classify_haproxy_as_node_on_shared_port(monkeypatch) -> None:
    def fake_http_get_details(url: str, timeout: float, retries: int = 1, **kwargs) -> dict[str, object]:
        _ = (timeout, retries, kwargs)
        if url.endswith("/metrics"):
            return {
...
def test_scan_keeps_generic_prometheus_metrics_unknown_with_multiple_profiles(monkeypatch) -> None:
    def fake_http_get_details(url: str, timeout: float, retries: int = 1, **kwargs) -> dict[str, object]:
        _ = (timeout, retries, kwargs)
        if url.endswith("/metrics"):
            return {
